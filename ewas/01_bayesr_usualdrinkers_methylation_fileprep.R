...
alcohol <- read.delim(paste0(datadir, "alcohol_10506_usualdrinkers.tsv"), row.names = 2)
...
# Export residualized
write.table(x = t(as.matrix(as.numeric(scale(alcohol$alcohol_units_log_resid)))),file = paste0(datadir, "allindividuals_10506_logalcohol_residualized_usualdrinkers.csvphen") ,quote = F, sep = ",", row.names = F, col.names = F)

# One less sample
alcohol <- alcohol[2:nrow(alcohol),]
write.table(x = t(as.matrix(as.numeric(scale(alcohol$alcohol_units_log_resid)))),file = paste0(datadir, "allindividuals_10506_logalcohol_residualized_usualdrinkers_onelesssample.csvphen") ,quote = F, sep = ",", row.names = F, col.names = F)
...
foreach(i=1:22, .packages = "data.table") %dopar% { 
  ...
  fwrite(meth, paste0(datadir, "methylation/GS20k_chr", i, "_resid_mvals_usualdrinkers_nogeno_somecovars.txt"),row.names=F)
...
files <- list.files(path = paste0(datadir, "methylation/"), pattern = "_resid_mvals_usualdrinkers_nogeno_somecovars.txt") # Get files
files <- files[order(files)]
data <- rbindlist(lapply(paste0(datadir, "methylation/", files),fread))
gc()

# Export fused methylation file - scaled
fwrite(x = as.matrix(data), paste0(datadir, "methylation/GS20k_allchrom_usualdrinkers_nogeno_somecovars.csv"), sep = ",", row.names = F, col.names = F, quote = F) 
gc()

# Re-import and remove one sample
data <- fread(paste0(datadir, "methylation/GS20k_allchrom_usualdrinkers_nogeno_somecovars.csv"))
data <- as.data.frame(data)
data <- data[,2:ncol(data)]
fwrite(x = as.matrix(data), paste0(datadir, "methylation/GS20k_allchrom_usualdrinkers_nogeno_somecovars_onelesssample.csv"), sep = ",", row.names = F, col.names = F, quote = F) 
gc()
