...
gsw4_en_filt = pd.read_table("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/predictions_gs_w4_usualdrinkerpredictor_noadjustments_scaledmeth_filteredcpgs.tsv", index_col = 0)
...
## Scatterplot for each category (usual, frequent, less), tested in w4, just for usual drinkers
col = set_colors(3, palette)
sns.set_palette(sns.color_palette(col))
true_col = "alcohol_units"
pred_col = "ac_pred"
df = gs_target_cats
r = scipy.stats.pearsonr(df[pred_col], df[true_col]) # 0.39885894225016205
...
sns.kdeplot(y = pred_col, x = true_col, data = df, ax = axes, color = lighten_color(col[0], 1.3), levels = 5, thresh=0.025)
...
sns.kdeplot(y = pred_col, x = true_col, data = df, ax = axes, color = lighten_color(col[1], 1.3), levels = 5, thresh=0.025)
...
sns.kdeplot(y = pred_col, x = true_col, data = df, ax = axes, color = lighten_color(col[2], 1.3), levels = 5, thresh=0.025)
sns.despine(offset=10, trim=True);
...
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_gsw4_categories.pdf", dpi = 300)
plt.close(fig)

## Scatterplot for each category (usual, frequent, less), tested in w4, just for usual drinkers, SEPARATE PLOTS
...
sns.despine(offset=10, trim=True);
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_gsw4_categories_separate.pdf", dpi = 300)
plt.close(fig)


#### CATEGORIES DIFFERENCES - ELNET | MATCHING SAMPLE SIZES
#####################################################################################
#####################################################################################

# Match sample sizes
usual = gs_target_cats.loc[(gs_target_cats["alcohol_usual"] == 2),].sample(n = 834)
more = gs_target_cats.loc[gs_target_cats["alcohol_usual"] == 1,].sample(n = 834)
less = gs_target_cats.loc[gs_target_cats["alcohol_usual"] == 3,]
gs_sampled = pd.concat([usual, more, less], axis = 0)
...
r = scipy.stats.pearsonr(df[pred_col], df[true_col]) # 0.39885894225016205
fig, axes = plt.subplots(1, 1, figsize = (8, 4))
df = gs_sampled.loc[gs_sampled["alcohol_usual"] == 2,]
r = scipy.stats.pearsonr(df[pred_col], df[true_col])
sns.regplot(y = pred_col, x = true_col, data = df, ax = axes, color = col[0], line_kws={"color": lighten_color(col[0], 1.3)}, scatter_kws={'alpha':0.5, 'linewidth':0, 'edgecolor':None, 'rasterized':True}, label = "Usual (r = %s)" % round(r[0], 3))
df = gs_sampled.loc[gs_sampled["alcohol_usual"] == 1,]
r = scipy.stats.pearsonr(df[pred_col], df[true_col])
sns.regplot(y = pred_col, x = true_col, data = df, ax = axes, color = col[1], line_kws={"color": lighten_color(col[1], 1.3)}, scatter_kws={'alpha':0.5, 'linewidth':0, 'edgecolor':None, 'rasterized':True}, label = "More (r = %s)" % round(r[0], 3))
df = gs_sampled.loc[gs_sampled["alcohol_usual"] == 3,]
r = scipy.stats.pearsonr(df[pred_col], df[true_col])
sns.regplot(y = pred_col, x = true_col, data = df, ax = axes, color = col[2], line_kws={"color": lighten_color(col[2], 1.3)}, scatter_kws={'alpha':0.5, 'linewidth':0, 'edgecolor':None, 'rasterized':True}, label = "Less (r = %s)" % round(r[0], 3))
sns.despine(offset=10, trim=True);
plt.legend(frameon = False)
...
plt.title("EpiScore across categories")
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_gsw4_categories_matchsize.pdf", dpi = 300)
plt.close(fig)
...
sns.despine(offset=10, trim=True);
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_gsw4_categories_separate_matchsize.pdf", dpi = 300)
plt.close(fig)
...
pred_21 = pd.read_table("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/predictions_lbc_1921_usualdrinkerspredictor_sexspecific_w1w3w4_noadjustments_scaledmeth.tsv", index_col = 0)
pred_36 = pd.read_table("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/predictions_lbc_1936_usualdrinkerspredictor_sexspecific_w1w3w4_noadjustments_scaledmeth.tsv", index_col = 0)
...
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_lbc_sexstratified.pdf", dpi = 300)
plt.close(fig)
...
sns.despine(offset=10, trim=True);
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_lbc_sexstratified_separatecohorts.pdf", dpi = 300)
plt.close(fig)

#### SEX SPECIFIC MODELS - ELNET | MATCH SAMPLE SIZE
#####################################################################################

pred_21 = pd.read_table("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/predictions_lbc_1921_usualdrinkerspredictor_sexspecific_w1w3w4_noadjustments_subset4410_scaledmeth.tsv", index_col = 0)
pred_36 = pd.read_table("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/predictions_lbc_1936_usualdrinkerspredictor_sexspecific_w1w3w4_noadjustments_subset4410_scaledmeth.tsv", index_col = 0)
pred_all = pd.concat([pred_21, pred_36], axis = 0)
...
plt.title("Sex-specific EpiScore")
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_lbc_sexstratified_matchedsamplesize.pdf", dpi = 300)
plt.close(fig)
...
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_lbc_sexstratified_matchedsamplesize_separatecohorts.pdf", dpi = 300)
plt.close(fig)
...
# Import predictions
lbc_predictions_36 = pd.read_table("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/predictions_lbc_1936_usualdrinkerspredictor_w1w3w4_noadjustments_scaledmeth.tsv", index_col = 0)
lbc_predictions_21 = pd.read_table("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/predictions_lbc_1921_usualdrinkerspredictor_w1w3w4_noadjustments_scaledmeth.tsv", index_col = 0)
...
roc_auc_21 = metrics.auc(fpr_21, tpr_21) # 0.8629972178359275
fpr_36, tpr_36, threshold_36 = metrics.roc_curve(lbc_target_36["alcohol_cat_bi"], lbc_predictions_36["ac_pred"])
roc_auc_36 = metrics.auc(fpr_36, tpr_36) # 0.7646532438478747
...
plt.tight_layout()
plt.savefig("/Cluster_Filespace/Marioni_Group/Elena/alcohol_consumption/results/usualdrinkers_test/prediction_lbc_heavydrinkers_auc.pdf", dpi = 300)
plt.close(fig)
